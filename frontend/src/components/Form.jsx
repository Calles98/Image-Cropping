...
function Form({ condition, setCondition, holeId, setHoleId, from, setFrom, to, setTo }) {
 ...
 //console.log('Form Data:', projectName, holeId, from, to);
  ...
  return (
    <div className='w-full p-4 bg-slate-100 rounded-md'>
      ...
      <h3 className='text-sm text-slate-600'>Please fill in the details below:</h3>
        <form className='flex flex-col gap-2' action="" method="post">
           
            <label htmlFor="">Hole ID:</label>
            <input type="text" name='hole-id' className='outline-none border border-solid rounded-md border-slate-700' onChange={(e) => setHoleId(e.target.value)} />
            <label htmlFor="">Core Condition:</label>
